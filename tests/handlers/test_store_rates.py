...
@pytest.mark.parametrize(
    "event,db_response",
    (
        ({"input": 1}, [{'date': '2023-04-28', 'kind': 'rates', 'rates': '{"USD": 1.0981, "GBP": 0.8805, "PLN": 4.5815}'}]),
        (
            {"limit": 5}, [
                {'date': '2023-04-28', 'kind': 'rates', 'rates': '{"USD": 1.0981, "GBP": 0.8805, "PLN": 4.5815}'},
                {'date': '2023-04-27', 'kind': 'rates', 'rates': '{"USD": 1.0911, "GBP": 0.8905, "PLN": 4.5815}'}
            ]
        )
    )
)
...
def test_store_rates_no_new_rates_daily(dynamodb_resource, xml_rates, lambda_context):
    ...
    with pytest.raises(NoNewRatesError):
        store_rates_handler(event={"limit": 1, "type": "daily-cron"}, context=lambda_context)
